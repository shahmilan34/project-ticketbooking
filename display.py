...
class Display(object):
    ...
    def displayMovies(self, movieList):
        ...
        while True:
            movie_id = int(input("Please enter Movie id from above list:\n"))
    ...
    def displayDates(self, movie_id, movieList, movieDates):
        ...
        while True:
            movies_dates_id = int(
                input("Please enter date id from above list:\n"))
            if (movies_dates_id in movieDates):
                print("\n selected "+movieList[movie_id]+" movie on " +
                      str(movieDates[movies_dates_id])+" date \n")
    ...
    def displayTimes(
            self, movie_id, movies_dates_id,
            movieList, movieDates, movieTimes
    ):
        ...
        while True:
            movies_time_id = int(
                input("Please enter time id from above list:\n"))
            if (movies_time_id in movieTimes):
                print(
                    "\n selected "+movieList[movie_id]+" movie on date "
                    + str(movieDates[movies_dates_id])+" and time "
                    + str(movieTimes[movies_time_id])+" \n"
                    )
    ...
    def displaySeats(self, bookedTicketNo):
        ...
        while True:
            ...
            if (seat_no in bookedTicketNo or seat_no > 50 or seat_no < 1):
                ...
                decision = input("Want to book another seat no type Y or N : ")
                if decision == 'Y':
                    continue
                return seat_nos
